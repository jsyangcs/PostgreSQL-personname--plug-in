                   ...
                   Count tk, Count tm, Count pm, Count bm)
{
	...
	//DONE
	Count nbSigs = psigBits(r);
	//Count bSigSize = r->params.bsigSize;
	Count bSigPP = maxBsigsPP(r);
	Count bSigNpages = nbSigs/bSigPP + 1;
	p->bsigNpages = bSigNpages;
	p->nbsigs = nbSigs;
	for (int i = 0; i < bSigNpages - 1; i++) {
		//addPage(r->bsigf);
		Page np = newPage();
		for (int j = 0; j < bSigPP; j++) {
			addOneItem(np);
		}
		putPage(r->bsigf, i, np);
	}
	//addPage(r->bsigf);
	Page np = newPage();
	Count rbSigs = nbSigs - bSigPP * (bSigNpages - 1);
	for (int j = 0; j < rbSigs; j++) {
		addOneItem(np);
	}
	putPage(r->bsigf, bSigNpages - 1, np);
...
PageID addToRelation(Reln r, Tuple t)
{
	...
	Count tSigSize = rp->tsigSize;
	Offset tSigPos = tSigSize * pageNitems(tSigPage);
	putBits(tSigPage, tSigPos, tSig);
	...
	//Bits npSig = NULL;	// the result psig
	PageID pSigPid = rp->psigNpages - 1;
	Page pSigPage = getPage(r->psigf, pSigPid);
	Count pSigSize = rp->psigSize;
	Offset pSigPos = 0;
	// check if new data page is added; if so add new psig 
	if (nPages(r) != nPsigs(r)) {
		// check if room on last page; if not add new page
		...
		pSigPos = pSigSize * pageNitems(pSigPage);
		putBits(pSigPage, pSigPos, pSig);
		rp->npsigs++;  //written to disk in closeRelation()
		addOneItem(pSigPage);
		putPage(r->psigf, pSigPid, pSigPage);
	}
	else {
		pSigPos = pSigSize * (pageNitems(pSigPage) - 1);
		Bits pPSig = newBits(psigBits(r));
		getBits(pSigPage, pSigPos, pPSig);
		orBits(pPSig, pSig);
		putBits(pSigPage, pSigPos, pPSig);
		putPage(r->psigf, pSigPid, pSigPage);
		freeBits(pPSig);
	}
	freeBits(pSig);
	...
	//PageID pid = rp->npages-1;
	Bits pSig_ = makePageSig(r, t);
	Bits bSig = newBits(bsigBits(r));
	PageID bSigPid = 0;
	Count bSigNo = 0;
	Page bSigPage = NULL;
	Count bSigSize = rp->bsigSize;
	Offset bSigPos = 0;
