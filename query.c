...
void scanAndDisplayMatchingTuples(Query q)
{
	assert(q != NULL);
	//DONE
	for (int i = 0; i < q->rel->params.npages; i++) {
		q->curpage = i;
		if (!bitIsSet(q->pages, i)) {
			continue;
		}
		int falseMatch = TRUE;
		Page curPage = getPage(q->rel->dataf, i);
		for (int j = 0; j < pageNitems(curPage); j++) {
			q->curtup = j;
			Tuple curTuple = getTupleFromPage(q->rel, curPage, j);
			if (tupleMatch(q->rel, curTuple, q->qstring)) {
				printf("%s\n", curTuple);
				falseMatch = FALSE;
			}
			free(curTuple);
